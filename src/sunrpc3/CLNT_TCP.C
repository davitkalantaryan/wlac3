...
static int	readtcp(REGISTER2 struct ct_data *ct,caddr_t buf, REGISTER2 int len);
static int	writetcp(struct ct_data *ct,caddr_t buf,int len);

extern void sun_rpc_report(const char* lpszMsg);
extern int bindresvport(int sd, struct sockaddr_in * sin);

static enum clnt_stat	clnttcp_call(REGISTER2 CLIENT *h,u_long proc,xdrproc_t xdr_args,caddr_t args_ptr,xdrproc_t xdr_results,caddr_t results_ptr,struct timeval timeout);
static void				clnttcp_abort(CLIENT *h);
...
{
	CLIENT *h;
	REGISTER2 struct ct_data *ct=NULL;
	struct timeval now;
	...
	if (h == NULL) {
#ifdef _WIN32
		sun_rpc_report("clnttcp_create: out of memory\n");
...
clnttcp_freeres(cl, xdr_res, res_ptr)
	...
	caddr_t res_ptr;
{
	REGISTER2 struct ct_data *ct = (struct ct_data *)cl->cl_private;
	REGISTER2 XDR *xdrs = &(ct->ct_xdrs);
