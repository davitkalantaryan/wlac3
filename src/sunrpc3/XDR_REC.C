...
static bool_t flush_out(REGISTER2 RECSTREAM* rstrm, bool_t eor);
static bool_t  set_input_fragment(REGISTER2 RECSTREAM *rstrm);
static bool_t  get_input_bytes(REGISTER2 RECSTREAM * rstrm, REGISTER2 caddr_t addr, REGISTER2 int len);
static bool_t  skip_input_bytes(REGISTER2 RECSTREAM * rstrm, long cnt);
	...
	int (*writeit)();  /* like write, but pass it a tcp_handle, not sock */
{
	...
	if (rstrm == NULL) {
#ifdef _WIN32
		sun_rpc_report("xdrrec_create: out of memory\n");
#else
...
xdrrec_getbytes(xdrs, addr, len)
	XDR *xdrs;
	REGISTER2 caddr_t addr;
	REGISTER2 u_int len;
{
	REGISTER2 RECSTREAM *rstrm = (RECSTREAM *)(xdrs->x_private);
	REGISTER2 int current;
	...
	register XDR *xdrs;
{
	...
	if (pos != -1)
		...
		case XDR_ENCODE:
			pos += (long)(rstrm->out_finger - rstrm->out_base);
			break;

		case XDR_DECODE:
			pos -= (long)(rstrm->in_boundry - rstrm->in_finger);
			break;
