	...
	xdrproc_t inproc, outproc;
{
	...
	if (transp == 0) {
		...
		if (transp == NULL) {
#ifdef _WIN32
			sun_rpc_report("couldn't create an rpc server\n");
#else
...
#ifdef _WIN32
	    	sun_rpc_report("couldn't register prog");
#else
	...
	if (pl == NULL) {
#ifdef _WIN32
		sun_rpc_report("registerrpc: out of memory\n");
#else
...
#ifdef _WIN32
	sun_rpc_report("never registered prog %d\n"/*, prog*/);
#else
