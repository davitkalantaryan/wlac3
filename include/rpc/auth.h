...
typedef struct AUTH_struct {
	...
	struct auth_ops {
		...
		void	(*ah_destroy)(REGISTER2 struct AUTH_struct*);	/* destroy this structure */
	} *ah_ops;
	caddr_t ah_private;
} AUTH;
...
extern AUTH * authunix_create(char* machname, int uid, int gid, REGISTER2 int len, unsigned int* aup_gids);
